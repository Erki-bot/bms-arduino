...
void setup() {
  ...
  delay(200);
  if (ledIndicatorHandler) xTaskNotifyGive(ledIndicatorHandler);
...
void loop() {
  ...
  if (client.connected()) client.publish(mqtt_data_topic, message);
    ...
    while (1) {
        ...
        if (WiFi.status() == WL_CONNECTED) {
            if (client.connected()) {
                ...
                if (blinkBlueLedHandler == NULL) { // Create task only if it doesn't exist
                    if (xTaskCreate(
                        ...
                        while (1); // Halt execution to prevent further issues
                    }
                }
            } else {
                // Blink both LEDs
                ...
                if (blinkBothLedsHandler == NULL) { // Create task only if it doesn't exist
                    if (xTaskCreate(
